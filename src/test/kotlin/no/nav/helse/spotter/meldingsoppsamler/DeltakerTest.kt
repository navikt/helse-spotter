...
internal class DeltakerTest {
    ...
    fun `uten participating services`() {
        ...
        assertEquals(Deltaker(navn = "Ukjent", tidspunkt = LocalDateTime.parse(tidspunkt)), jacksonObjectMapper().readTree(json).deltaker())
    ...
    fun `med participating services`() {
        ...
        """.trimIndent()
        assertEquals(Deltaker(navn = "behovsakkumulator", tidspunkt = LocalDateTime.parse(tidspunkt)), jacksonObjectMapper().readTree(json).deltaker())
